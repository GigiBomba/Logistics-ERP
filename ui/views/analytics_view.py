...
CHART_PRIMARY = COLORS.get("chart_primary", "#6366f1")
CHART_SECONDARY = COLORS.get("chart_secondary", "#818cf8")
...
class QtAnalyticsView(QScrollArea):
    ...
    def _build_controls(self, layout: QVBoxLayout) -> None:
        ...
        prev_btn.setProperty("role", "nav-toggle")
        ...
        to_lbl.setProperty("fontRole", "label")
        ...
        apply_btn = ActionButton(
            bar, t("analytics.apply"), self._apply_custom, variant="primary",
        )
    ...
    def _render_charts(self, data) -> None:
        ...
        ax1 = axes[0, 0]
        ax1.set_facecolor(COLORS.get("bg_base", "#09090b"))
        if trucks:
            ...
            colors = [CHART_PRIMARY] * len(names)
            if profits:
            ...
            ax1.text(0.5, 0.5, t("common.no_data"), ha="center", va="center",
                     transform=ax1.transAxes, color=COLORS.get("text_muted", "#aaa"))

        title1 = ax1.set_title(t("analytics.top_trucks_title"), color=COLORS.get("text_primary", "#fff"), fontsize=10)
        self._chart_texts.append((title1, "analytics.top_trucks_title"))
        ax1.tick_params(colors=COLORS.get("text_secondary", "#888"), labelsize=8)

        for spine in ax1.spines.values():
            spine.set_edgecolor(COLORS.get("border", "#333"))

        # ── Chart 2: Revenue vs Expenses (line) ──
        ax2 = axes[0, 1]
        ax2.set_facecolor(COLORS.get("bg_base", "#09090b"))
        ...
        if months and revenue and expenses:
            ...
            ax2.plot(idx, revenue, color=CHART_PRIMARY, label=t("analytics.revenue_label"), linewidth=2)
            ax2.fill_between(idx, 0, revenue, alpha=0.15, color=CHART_PRIMARY)
            ax2.plot(idx, expenses, color=CHART_SECONDARY, label=t("analytics.expenses_label"), linewidth=2)
            ax2.fill_between(idx, 0, expenses, alpha=0.15, color=CHART_SECONDARY)
            ax2.set_xticks(idx)
            ax2.set_xticklabels(months, rotation=45, ha="right", fontsize=7)
            ax2.legend(loc="upper left", fontsize=7)
        else:
            ax2.text(0.5, 0.5, t("common.no_data"), ha="center", va="center",
                     transform=ax2.transAxes, color=COLORS.get("text_muted", "#aaa"))

        title2 = ax2.set_title(
            t("analytics.revenue_expenses_title"), color=COLORS.get("text_primary", "#fff"), fontsize=10,
        )
        self._chart_texts.append((title2, "analytics.revenue_expenses_title"))
        ax2.tick_params(colors=COLORS.get("text_secondary", "#888"), labelsize=8)
        for spine in ax2.spines.values():
            spine.set_edgecolor(COLORS.get("border", "#333"))

        # ── Chart 3: Profit per Driver (bar) ──
        ax3 = axes[1, 0]
        ax3.set_facecolor(COLORS.get("bg_base", "#09090b"))
        if drivers:
            ...
            colors_d = [CHART_PRIMARY] * len(dnames)
            if dprofits:
            ...
            ax3.text(0.5, 0.5, t("common.no_data"), ha="center", va="center",
                     transform=ax3.transAxes, color=COLORS.get("text_muted", "#aaa"))

        title3 = ax3.set_title(
            t("analytics.driver_profit_title"), color=COLORS.get("text_primary", "#fff"), fontsize=10,
        )
        self._chart_texts.append((title3, "analytics.driver_profit_title"))
        ax3.tick_params(colors=COLORS.get("text_secondary", "#888"), labelsize=8)
        for spine in ax3.spines.values():
            spine.set_edgecolor(COLORS.get("border", "#333"))

        # ── Chart 4: Profit/Expenses Ratio (pie) ──
        ax4 = axes[1, 1]
        ax4.set_facecolor(COLORS.get("bg_base", "#09090b"))
        ...
        if total_profit > 0 or total_exp > 0:
            ...
            ax4.pie(sizes, labels=labels_pie, autopct="%1.0f%%",
                    colors=[CHART_PRIMARY, CHART_SECONDARY],
                    textprops={"color": COLORS.get("text_primary", "#fff"), "fontsize": 8})
        else:
            ax4.text(0.5, 0.5, t("common.no_data"), ha="center", va="center",
                     transform=ax4.transAxes, color=COLORS.get("text_muted", "#aaa"))

        title4 = ax4.set_title(t("analytics.profit_ratio_title"), color=COLORS.get("text_primary", "#fff"), fontsize=10)
