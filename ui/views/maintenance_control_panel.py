    ...
    def _build_kpi_row(self, layout):
        ...
        for key, title_key in kpi_defs:
            ...
            if key == "avg_health":
                ...
                pb.setFixedHeight(4)
                pb.setStyleSheet(f"""
                    QProgressBar {{ background: {COLOR_BG_OVERLAY}; border: none; border-radius: {RADIUS_XS}px; }}
                    QProgressBar::chunk {{ background: {COLOR_SUCCESS_DEFAULT}; border-radius: {RADIUS_XS}px; }}
                """)
            ...
            if key == "avg_health":
                ...
                if hasattr(self, "_health_progress"):
                    ...
                    self._health_progress.setStyleSheet(f"""
                        QProgressBar {{ background: {COLOR_BG_OVERLAY}; border: none; border-radius: {RADIUS_XS}px; }}
                        QProgressBar::chunk {{ background: {color}; border-radius: {RADIUS_XS}px; }}
                    """)
