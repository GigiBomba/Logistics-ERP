...
class MapWidget(QWebEngineView):
    ...
    def _bridge_script(map_var: str, qwc_js: str) -> str:
        return (
            ...
            "function _initClickBridge() {\n"
            "  if (typeof _opmap === 'undefined' || !_opmap || typeof _opmap.on !== 'function') {\n"
            "    setTimeout(_initClickBridge, 100);\n"
            "    return;\n"
            "  }\n"
            "  try {\n"
            ...
            "        try { window._pybridge.map_click(e.latlng.lat, e.latlng.lng); } catch(ex) {}\n"
            "      });\n"
            "    });\n"
            "  } catch(e) {\n"
            "    setTimeout(_initClickBridge, 200);\n"
            "  }\n"
            "}\n"
            "if (document.readyState === 'loading') {\n"
            "  document.addEventListener('DOMContentLoaded', _initClickBridge);\n"
            "} else {\n"
            "  _initClickBridge();\n"
            "}\n"
            "function _opReady() { return typeof _opmap !== 'undefined' && _opmap !== null && typeof _opmap.eachLayer === 'function'; }\n"
            "function _opAddMarker(lat, lng, label, color) {\n"
            "  if (!_opReady()) return null;\n"
