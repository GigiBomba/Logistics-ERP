...
def generate_efti_xml(
    ...
    # Carrier — prefer trip_data carrier fields, fall back to company_config
    carrier_name    = trip_data.get("carrier_name") or company_config.get("company_name", "")
    carrier_addr    = trip_data.get("carrier_address") or company_config.get("address", "")
    carrier_country = trip_data.get("carrier_country", "")
    carrier_city    = trip_data.get("carrier_city", "")
    carrier_vat     = trip_data.get("carrier_vat") or company_config.get("cui", "")
    carrier_eori    = trip_data.get("carrier_eori") or company_config.get("eori_number", "")
    ...
    # NOTE: we store only a boolean "presented" in XML; the raw filesystem path
    # is never written to the output — only the filename is used as indicator.
    sig_sender_path = trip_data.get("sig_sender_path", "")
    if sig_sender_path and os.path.isfile(sig_sender_path):
    ...
    if sig_carrier_path and os.path.isfile(sig_carrier_path):
        sig_doc_auth = _sub(doc, f"{{{NS_RAM}}}SignatoryDocumentAuthentication")
        _sub(sig_doc_auth, f"{{{NS_RAM}}}SignatoryAuthentication",
             "presented", {"authenticationTypeCode": "DigitalSignature"})
    ...
    if sig_consignee_path and os.path.isfile(sig_consignee_path):
