...
import { Lightbulb, Target, Trophy, Filter, AlertCircle } from "lucide-react"
...
import { useRoadmap } from "@/services/queries"
...
const statusConfig: Partial<Record<RoadmapStatus, { labelKey: string; className: string }>> = {
...
const statusColumnConfig: Partial<Record<RoadmapStatus, { titleKey: string; descKey: string }>> = {
...
function RoadmapCard({ item, index, t }: { item: RoadmapItem; index: number; t: (key: string) => string }) {
  ...
  const status = statusConfig[item.status] ?? { labelKey: "roadmap.planned", className: "bg-muted text-muted-foreground" }
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ duration: 0.35, delay: prefersReducedMotion ? 0 : Math.min(index * 0.05, 0.3), ease: [0.22, 1, 0.36, 1] }}
    >
...
export default function RoadmapPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items
  }, [roadmapData, activeStatusFilter, activeCategoryFilter])
  ...
  return (
    <>
      <SeoHead title={t("roadmap.pageTitle")} description={t("roadmap.metaDesc")} canonical="https://operionerp.xyz/roadmap" />
      ...
      {isLoading ? (
        ...
        <SectionWrapper>
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h2 className="text-xl font-semibold">{t("roadmap.failedToLoad")}</h2>
            <p className="mt-2 text-muted-foreground">
              {t("roadmap.failedToLoadDesc")}
            </p>
          </div>
        </SectionWrapper>
      ) : (
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    {recentlyCompleted.map((item, i) => (
                      <motion.div
                        ...
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ delay: prefersReducedMotion ? 0 : Math.min(i * 0.05, 0.3) }}
                      >
                        <Card className="h-full bg-background">
                          <CardContent className="p-4">
                            ...
                            {item.target_date && (
                              <p className="mt-1 text-xs text-muted-foreground">{formatDate(item.target_date)}</p>
                            )}
                          </CardContent>
                        </Card>
                      </motion.div>
          ...
          <SectionWrapper>
            {roadmapData && roadmapData.length === 0 ? (
              <div className="mx-auto max-w-xl text-center py-12">
                <p className="text-sm text-muted-foreground">{t("roadmap.noItems")}</p>
              </div>
            ) : showColumnLayout ? (
              <div className="mx-auto max-w-5xl">
