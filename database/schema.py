...
# ── Trans.eu Domain (Phase 3): orders, negotiation, dock, providers ─────────
# Local mirror of the Trans.eu domain objects (TransEU_Architecture.md §5.1 /
# §9.13; TransEU_KnowledgeBase.md §6.8/§6.9/§7.2-§7.7).  JSON fields store the
# raw Trans.eu KB object shapes as TEXT in SQLite (JSONB in schema_pg.sql).
# The Phase-1 trans_eu_* tables (trans_eu_user_tokens, trans_eu_freight_offers,
# trans_eu_webhook_events, trans_eu_webhook_events_failed — Alembic
# a9b0c1d2e3f5) and freight_negotiations are NOT re-created here.
TABLE_FREIGHT_ORDERS = """
CREATE TABLE IF NOT EXISTS freight_orders (
    id TEXT PRIMARY KEY,                      -- UUID, generated by Python for SQLite
    company_id INTEGER NOT NULL REFERENCES companies(id),
    trans_eu_order_id TEXT NOT NULL,          -- Trans.eu order UUID (KB §7.2)
    trans_eu_freight_id INTEGER NOT NULL,
    order_number TEXT DEFAULT '',
    status TEXT NOT NULL DEFAULT 'created',   -- created | confirmed | cancelled | delivered
    price_amount REAL,
    price_currency TEXT DEFAULT 'EUR',
    payment_type TEXT DEFAULT '',
    execution_data TEXT,                      -- JSON: entry execution data (KB §6.4.4)
    linked_trip_id INTEGER REFERENCES trips(id),  -- canonical Operion trip
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at TEXT NOT NULL DEFAULT (datetime('now')),
...
INDEX_FREIGHT_ORDERS_COMPANY = (
    "CREATE INDEX IF NOT EXISTS idx_freight_orders_company "
    "ON freight_orders(company_id);"
)
INDEX_FREIGHT_ORDERS_FREIGHT_ID = (
    "CREATE INDEX IF NOT EXISTS idx_freight_orders_freight_id "
    "ON freight_orders(company_id, trans_eu_freight_id);"
)
INDEX_FREIGHT_ORDERS_TRIP = (
    "CREATE INDEX IF NOT EXISTS idx_freight_orders_trip "
    "ON freight_orders(linked_trip_id);"
)

TABLE_NEGOTIATION_OFFERS = """
CREATE TABLE IF NOT EXISTS negotiation_offers (
    id TEXT PRIMARY KEY,                      -- UUID, generated by Python for SQLite
    company_id INTEGER NOT NULL REFERENCES companies(id),
    trans_eu_freight_id INTEGER NOT NULL,
    offer_id TEXT NOT NULL,
    direction TEXT NOT NULL DEFAULT 'inbound',  -- inbound (received) | outbound (our reply)
    status TEXT NOT NULL DEFAULT 'negotiation', -- acceptation | negotiation | rejection | renouncement
    price REAL,
    currency TEXT DEFAULT 'EUR',
    counterparty_name TEXT DEFAULT '',
    counterparty_id TEXT DEFAULT '',
    author TEXT DEFAULT '',
    parent_offer_id TEXT REFERENCES negotiation_offers(id),  -- self-FK thread chain
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at TEXT NOT NULL DEFAULT (datetime('now')),
...
INDEX_NEGOTIATION_OFFERS_FREIGHT = (
    "CREATE INDEX IF NOT EXISTS idx_negotiation_offers_freight "
    "ON negotiation_offers(company_id, trans_eu_freight_id);"
)

TABLE_DOCK_WAREHOUSES = """
CREATE TABLE IF NOT EXISTS dock_warehouses (
    id TEXT PRIMARY KEY,                      -- UUID, generated by Python for SQLite
    company_id INTEGER NOT NULL REFERENCES companies(id),
    trans_eu_warehouse_id INTEGER NOT NULL,
    name TEXT DEFAULT '',
    address TEXT,                             -- JSON: {address, coordinates, ...}
    ramps TEXT,                               -- JSON: [{id, name, ramp_type}, ...]
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at TEXT NOT NULL DEFAULT (datetime('now')),
...
INDEX_DOCK_WAREHOUSES_COMPANY = (
    "CREATE INDEX IF NOT EXISTS idx_dock_warehouses_company "
    "ON dock_warehouses(company_id);"
)

TABLE_DOCK_TIME_WINDOWS = """
CREATE TABLE IF NOT EXISTS dock_time_windows (
    id TEXT PRIMARY KEY,                      -- UUID, generated by Python for SQLite
    company_id INTEGER NOT NULL REFERENCES companies(id),
    trans_eu_window_id INTEGER NOT NULL,
    warehouse_id INTEGER NOT NULL,            -- Trans.eu warehouse id
    valid_from TEXT,                          -- date (YYYY-MM-DD)
    valid_to TEXT,                            -- date (YYYY-MM-DD)
    start_time TEXT,                          -- time (HH:MM:SS)
    end_time TEXT,                            -- time (HH:MM:SS)
    range_type TEXT DEFAULT '',
    external_number TEXT DEFAULT '',
    carrier_id INTEGER,
    purchase_order TEXT,                      -- JSON: KB §7.6 purchase_order
    route TEXT,                               -- JSON: KB §7.6 route
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at TEXT NOT NULL DEFAULT (datetime('now')),
...
INDEX_DOCK_TIME_WINDOWS_WAREHOUSE = (
    "CREATE INDEX IF NOT EXISTS idx_dock_time_windows_warehouse "
    "ON dock_time_windows(company_id, warehouse_id);"
)

TABLE_DOCK_ANNOUNCEMENTS = """
CREATE TABLE IF NOT EXISTS dock_announcements (
    id TEXT PRIMARY KEY,                      -- UUID, generated by Python for SQLite
    company_id INTEGER NOT NULL REFERENCES companies(id),
    trans_eu_announcement_id INTEGER NOT NULL,
    reference_number TEXT DEFAULT '',
    status TEXT NOT NULL DEFAULT 'CONFIRMED', -- CONFIRMED | IN_PROGRESS | FINISHED | REFUSED
    stage TEXT DEFAULT 'Vehicle_Arrived',     -- Vehicle_Arrived | Started | Finished | Vehicle_Left
    date_from TEXT,                           -- date (YYYY-MM-DD)
    date_to TEXT,                             -- date (YYYY-MM-DD)
    operation_type TEXT DEFAULT '',           -- loading | unloading
    operation_time TEXT DEFAULT '',
    carrier_id INTEGER,
    carrier_name TEXT DEFAULT '',
    shipper_id INTEGER,
    driver TEXT,                              -- JSON: KB §7.5 driver object
    vehicle TEXT,                             -- JSON: KB §7.5 vehicle object
    ramp_id INTEGER,
    warehouse_id INTEGER,
    route TEXT,                               -- JSON: KB §7.5 route
    notes TEXT,                               -- JSON: KB §7.5 notes[]
...
INDEX_DOCK_ANNOUNCEMENTS_COMPANY = (
    "CREATE INDEX IF NOT EXISTS idx_dock_announcements_company "
    "ON dock_announcements(company_id);"
)
INDEX_DOCK_ANNOUNCEMENTS_STATUS = (
    "CREATE INDEX IF NOT EXISTS idx_dock_announcements_status "
    "ON dock_announcements(company_id, status);"
)

TABLE_PROVIDER_CONTRACTS = """
CREATE TABLE IF NOT EXISTS provider_contracts (
    id TEXT PRIMARY KEY,                      -- UUID, generated by Python for SQLite
    company_id INTEGER NOT NULL REFERENCES companies(id),
    trans_eu_contract_id TEXT NOT NULL,       -- Trans.eu route-contract UUID (KB §7.7)
    contract_type TEXT NOT NULL DEFAULT 'fixed',  -- fixed | flexible
    carrier_id INTEGER,
    carrier_name TEXT DEFAULT '',
    order_terms TEXT,                         -- JSON: KB §7.7 order_terms
    status TEXT DEFAULT 'registered',         -- KB §9.29 contract statuses
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at TEXT NOT NULL DEFAULT (datetime('now')),
...
INDEX_PROVIDER_CONTRACTS_COMPANY = (
    "CREATE INDEX IF NOT EXISTS idx_provider_contracts_company "
    "ON provider_contracts(company_id);"
)

TABLE_PROVIDER_PARTNERS = """
CREATE TABLE IF NOT EXISTS provider_partners (
    ...
    vat_id TEXT DEFAULT '',
    cooperation_status TEXT DEFAULT 'active', -- active | blocked
    groups TEXT,                              -- JSON: [{id, name, ...}]
    trans_eu_employee_ids TEXT,               -- JSON: [trans_id, ...]
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at TEXT NOT NULL DEFAULT (datetime('now')),
...
INDEX_PROVIDER_PARTNERS_COMPANY = (
    "CREATE INDEX IF NOT EXISTS idx_provider_partners_company "
    "ON provider_partners(company_id);"
)

TABLE_PROVIDER_VEHICLES = """
CREATE TABLE IF NOT EXISTS provider_vehicles (
    ...
    vin TEXT DEFAULT '',
    truck_trailer_plates TEXT,                -- JSON: [truck_plate, trailer_plate, ...]
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at TEXT NOT NULL DEFAULT (datetime('now')),
...
INDEX_PROVIDER_VEHICLES_COMPANY = (
    "CREATE INDEX IF NOT EXISTS idx_provider_vehicles_company "
    "ON provider_vehicles(company_id);"
)

TABLE_TRANS_EU_VEHICLE_OFFERS = """
CREATE TABLE IF NOT EXISTS trans_eu_vehicle_offers (
    id TEXT PRIMARY KEY,                      -- UUID, generated by Python for SQLite
    company_id INTEGER NOT NULL REFERENCES companies(id),
    trans_eu_offer_id INTEGER NOT NULL,
    vehicle_id INTEGER,                       -- Trans.eu fleet vehicle id the offer refers to
    offer_type TEXT DEFAULT '',
    available_from TEXT,                      -- date (YYYY-MM-DD)
    available_to TEXT,                        -- date (YYYY-MM-DD)
    origin TEXT,                              -- JSON: {country, locality, postal_code, ...}
    destination TEXT,                         -- JSON: {country, locality, postal_code, ...}
    price REAL,
    currency TEXT DEFAULT 'EUR',
...
INDEX_TRANS_EU_VEHICLE_OFFERS_COMPANY = (
    "CREATE INDEX IF NOT EXISTS idx_trans_eu_vehicle_offers_company "
    "ON trans_eu_vehicle_offers(company_id);"
)
