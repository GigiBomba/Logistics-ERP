...
class TestDocumentsRouter:
    ...
    def test_upload_document_success(self, client_with_mocks):
        ...
        mocks["document_service"].upload_document.return_value = MagicMock(
            success=True,
            data=MagicMock(model_dump=lambda: {**self.FAKE_DOC, "id": 10}),
        )
    ...
    def test_upload_document_with_metadata(self, client_with_mocks):
        ...
        mocks["document_service"].upload_document.return_value = MagicMock(
            success=True,
            data=MagicMock(model_dump=lambda: {**self.FAKE_DOC, "id": 20, "category": "invoices"}),
        )
        ...
        assert resp.status_code == 200
        mocks["document_service"].upload_document.assert_called_once()
        # upload_document receives (DocumentUpload, user_id=...)
        upload_arg = mocks["document_service"].upload_document.call_args[0][0]
        assert upload_arg.category == "invoices"
        assert upload_arg.entity_type == "trip"
        assert upload_arg.entity_id == 42
