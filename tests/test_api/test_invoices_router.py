"""Tests for the invoices API router (``/api/v1/invoices``)."""
...
class TestInvoicesRouter:
    """Invoice generation and email-sending endpoints."""

    # ── generate ───────────────────────────────────────────────────────────

    @patch("services.invoicing.service.InvoiceService")
    def test_generate_invoice_returns_pdf(
        self, mock_svc_cls, client_with_mocks, tmp_path
    ):
    ...
    @patch("services.invoicing.service.InvoiceService")
    def test_generate_invoice_returns_500_when_file_missing(
        self, mock_svc_cls, mock_isfile, client_with_mocks
    ):
        client, mocks = client_with_mocks
        mock_svc = MagicMock()
        mock_svc_cls.return_value = mock_svc
    ...
    @patch("services.invoicing.service.InvoiceService")
    def test_send_invoice_email_returns_sent(
    ...
    @patch("services.invoicing.service.InvoiceService")
    def test_send_invoice_email_returns_400_without_recipient(
        self, mock_svc_cls, client_with_mocks
    ):
        client, mocks = client_with_mocks
    ...
    @patch("services.invoicing.service.InvoiceService")
    def test_send_invoice_email_returns_400_on_value_error(
        self, mock_svc_cls, client_with_mocks
    ):
        client, mocks = client_with_mocks
        mock_svc = MagicMock()
    ...
    @patch("services.invoicing.service.InvoiceService")
    def test_send_invoice_email_returns_failed_when_not_ok(
        self, mock_svc_cls, client_with_mocks
    ):
        client, mocks = client_with_mocks
        mock_svc = MagicMock()
