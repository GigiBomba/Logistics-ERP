...
| 11 | UI_AUDIT_2026_FINDINGS_AND_PLAN.md | hybrid | 🟡 MOSTLY IMPLEMENTED | ~72% |
...
| 15 | TransEU_PhaseB_Implementation.md | blueprint | 🟡 MOSTLY IMPLEMENTED | ~70% |
| 16 | website/Operion_Waitlist_Blueprint_V1.md | blueprint | 🟡 MOSTLY IMPLEMENTED | ~75–80% |
...
### 11. UI_AUDIT_2026_FINDINGS_AND_PLAN.md — ~72% (hybrid; self-reported status inflated)
Real: STATUS_STYLES i18n, maintenance hub, component canonicalization (`ui/components.py`), collapsible filter sections, window min 1280×720 tokens, IBM Plex fonts, `tools/ui_style_gate.py`, evidence dirs. Contradicted claims: `PlaceholderView` still exists (`ui/main_window.py:1153`), `_PlaceholderPanel` still in automail_view, border-radius standardization not done, tokens SPACE_3/5 + BTN_HEIGHT unchanged, route planner still uses `EventBus()` not `shared_event_bus`, `board_state.py` has no QMutex.

### 12. ARCHITECTURALREWORK.md — ~85%
Distributed architecture largely delivered: backend/ with 56 routers + Pydantic schemas + middleware + factory; BackendSettings (postgres/redis/celery/env) exactly per spec; docker-compose (PostgreSQL 15 + Redis 7 + api/worker/beat/nginx/backup) + `.env.production`/`.env.development`; service-layer purity (zero `conn.execute` in backend/services); transaction ctx manager + BEGIN IMMEDIATE + no-auto-commit repos; Redis cache; Postgres pool + `?`→`%s` adaptation; client/ with 45+ ApiClient methods and all Remote* services; `main_remote.py` + MainWindow remote branch; API Dashboard tab. Gaps: no uv setup/lock; no root `.env.example`; FTS5→tsvector PG path incomplete; Phase-6 verification items not evidenced.
...
Layer-1 adapter + OAuth + token storage + webhook ingestion + polling sync + 5 celery tasks + status endpoints implemented. Missing by design (later phases): the 8 Trans.eu domain services (`services/trans_eu/` has only client/sync/webhook), domain tables (negotiation_offers, freight_orders, dock_*, provider_*) and Copilot tools (7 planned).

### 15. TransEU_PhaseB_Implementation.md — ~70%
OAuth loopback + connect view + connect endpoint + real search + webhook dispatch + sync service + 5 celery tasks + concurrency tests all verified. Missing: PhaseB's domain-service layer (8 files never created), domain tables absent, webhook→sync direct call chain incomplete (sync is polling-only — `_handle_trans_eu_webhook` stores/routes but never invokes `FreightSyncService`), webhook URL shape differs from architecture (`{partner}` vs `trans-eu/{company_id}`).

### 16. website/Operion_Waitlist_Blueprint_V1.md — ~75–80%
Frontend fully verified: honeypot (exact §2.1 spec), Turnstile + CSP, Zod validation, source tracking, analytics events, referral + WhatsApp share, live counter, admin UI (4 tabs, filters, CSV export, campaign w/ segment, role gate), newsletter form parity, GCM v2 cookie consent. Backend confirmed present (my follow-up check): `backend/api/v1/waitlist.py` — join/count/unsubscribe + admin CRUD/export/stats/campaign, 5-per-IP-10-min rate limit, `backend/services/email_provider.py` abstraction, schema + Turnstile verification. Gaps: §5.1 progressive-disclosure form (company_size/country/fleet_size fields) not on the live form; §5.3 contextual CTAs (route_calculator/profit_calculator/blog sources) never emitted.
...
### 22. website/Operion_Website_SGrade_Blueprint.md — ~40%
Strong frontend base: full public/admin page catalog, 17 UI primitives + 25 shared components, 6 i18n locales + audit script, cookie consent/GCM v2, Turnstile, security headers, sitemap/OG/robots/PWA, MFA + subscription + devices API layers, MSW. Not implemented per spec: token storage migration (still localStorage), MFA enrollment UI, Stripe checkout wiring, error-boundary Sentry, notification bell wiring, prerendering, visual-regression suite, critical-path E2E dir, k6 load tests, component a11y tests (only mutation config README), print stylesheet, browser matrix, DPA page. Several items are backend-dependent/deferred per its own P0–P3 phasing.
