...
def get_driver_tacho(
    ...
    Buckets are filled from the REAL ``tacho_driver_activity`` table (the same
    source as desktop ``GET /drivers/{driver_id}/tacho-activity``).  Every row
    supplies all four buckets; no synthetic data is invented.
    """
    company_id = current_user["company_id"]
    ...
    # tacho_driver_activity has no company_id column; the driver has already
    # been company-verified above (404 otherwise), so scoping by driver_id is
    # sufficient and safe.
