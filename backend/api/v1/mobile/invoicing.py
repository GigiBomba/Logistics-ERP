...
def transition_invoice(
    ...
    Action set (REAL machine targets): finalize → finalized, generate_xml →
    xml_generated, mark_paid → paid, cancel → cancelled.  Every transition is
    validated against ``INVOICE_STATUS_TRANSITIONS``; illegal ones are 422
    with a machine-readable ``error_code``, never 500.

    The ANAF submission chain was removed entirely (no ANAF anywhere) — the
    legacy ``submit`` action is rejected with 422 ``action_not_supported``
    BEFORE any permission gate so every caller gets the same machine-readable
    answer.
    """
    ...
    supported_actions = {"finalize", "generate_xml", "mark_paid", "cancel"}
