...
class RevenueTrendPoint(BaseModel):
    """One month of company-scoped revenue for the dispatcher trend."""
    month: str = ""
    revenue: float = 0.0


class ActivityItem(BaseModel):
    """One recent-activity row (trip or alert) for the dispatcher feed."""
    type: str = ""            # "trip" | "alert"
    id: int = 0
    title: str = ""
...
class ReassignRequest(BaseModel):
    """Reassign a transport to an active driver (dispatcher gate)."""
...
class InvoiceTransitionRequest(BaseModel):
    ...
    ``action`` is a free string — the supported set (finalize, generate_xml,
    mark_paid, cancel) is enforced server-side in the transition handler.
    Any other action (including the removed ANAF ``submit``) is rejected with
    a machine-readable 422 ``action_not_supported``.  The status machine
    itself is validated against the REAL ``INVOICE_STATUS_TRANSITIONS`` table
    (desktop ``_validate_status_transition``).
    """
